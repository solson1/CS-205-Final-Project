...
public class GameWrapper {
	...
	private LinkedList<AiObj> aiPlayerList;
	...
	public LinkedList<GameState> AIAction()
	{
		...
		for(AiObj ai: this.aiPlayerList)
		{
	 ...
	 * @return
	 */
	private LinkedList<AiObj> generateListOfAI(String[] playerNames, GameState state)
	{
		int numPlayers = playerNames.length;
		LinkedList<AiObj> AIPlayers = new LinkedList<AiObj>();
		for(int i = 0; i < numPlayers; i++)
		{
			//AIPlayers.add(new AiObj(state, playerNames[i], numPlayers));
			AIPlayers.add(new AiObj(state));
			
			AIPlayers.get(i).start(playerNames[i]);
		}
		return AIPlayers;
	}
